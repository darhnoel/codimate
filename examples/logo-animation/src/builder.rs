use crate::{
    view::build_logo_animation, LogoAnimation, LogoAnimationMotion, LogoAnimationTiming,
    LogoAnimationTrace,
};
...
type Inner = ExplanationBuilder<
    LogoAnimation,
    fn(LogoAnimation) -> LogoAnimationTrace,
    LogoAnimationMotion,
    LogoAnimationTiming,
>;
...
impl ExplainBuilder {
    pub fn state(self, state: LogoAnimation) -> Self {
        Self(self.0.state(state))
    }

    pub fn algorithm(self, algorithm: fn(LogoAnimation) -> LogoAnimationTrace) -> Self {
        Self(self.0.algorithm(algorithm))
    }

    pub fn motion(self, motion: fn() -> LogoAnimationMotion) -> Self {
        Self(self.0.motion(motion()))
    }

    pub fn timing(self, timing: LogoAnimationTiming) -> Self {
    ...
    pub fn build(self) -> (Box<dyn Playable>, Viewport) {
        ...
        let (state, algorithm, motion, timing) = self
            .0
            .take()
            .expect("logo-animation: state, algorithm, motion required");
        let viewport = Viewport::new(640.0, 480.0);
        let shaped = crate::LogoAnimationState::shape(state.clone(), viewport);
        build_logo_animation(name, algorithm(state), motion, timing, viewport, shaped)
    }
