...
fn main() {
    ...
    let explanation = explain("Faster Dijkstra")
    ...
    match preset {
        RenderPreset::Standard => explanation.render("results/faster-dijkstra.mp4"),
        RenderPreset::Hd60 => {
            explanation.render_with("results/faster-dijkstra-1080p60.mp4", |viewport| {
...
fn render_preset() -> RenderPreset {
    ...
    for arg in std::env::args().skip(1) {
        match arg.as_str() {
            ...
            "--help" | "-h" => {
                println!("usage: cargo run -p codimate-example-faster-dijkstra -- [--1080p60]");
                std::process::exit(0);
            }
            other => {
                eprintln!("unknown option: {other}");
                eprintln!("usage: cargo run -p codimate-example-faster-dijkstra -- [--1080p60]");
                std::process::exit(2);
            }
        }
    }
