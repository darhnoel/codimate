use crate::view::build_transformer;
use crate::{TransformerMotion, TransformerState, TransformerTiming, TransformerTrace};
...
type Inner = ExplanationBuilder<
    TransformerState,
    fn(TransformerState) -> TransformerTrace,
    TransformerMotion,
    TransformerTiming,
>;
...
impl ExplainBuilder {
    pub fn state(self, state: TransformerState) -> Self {
        Self(self.0.state(state))
    }

    pub fn algorithm(self, algorithm: fn(TransformerState) -> TransformerTrace) -> Self {
        Self(self.0.algorithm(algorithm))
    }

    pub fn motion(self, motion: fn() -> TransformerMotion) -> Self {
        Self(self.0.motion(motion()))
    }

    pub fn timing(self, timing: TransformerTiming) -> Self {
    ...
    pub fn build(self) -> (Box<dyn Playable>, Viewport) {
        ...
        let (state, algorithm, motion, timing) = self
            .0
            .take()
            .expect("transformer: state, algorithm, motion required");
        let trace = algorithm(state);
        build_transformer(name, trace, state.show_pulses, motion, timing)
    }
