use crate::{
    view::build_insertion_sort, InsertionSort, InsertionSortMotion, InsertionSortTiming,
    InsertionTrace,
};
...
type Inner = ExplanationBuilder<
    InsertionSort,
    fn(InsertionSort) -> InsertionTrace,
    InsertionSortMotion,
    InsertionSortTiming,
>;
...
impl ExplainBuilder {
    pub fn state(self, state: InsertionSort) -> Self {
        Self(self.0.state(state))
    }

    pub fn algorithm(self, algorithm: fn(InsertionSort) -> InsertionTrace) -> Self {
        Self(self.0.algorithm(algorithm))
    }

    pub fn motion(self, motion: fn() -> InsertionSortMotion) -> Self {
        Self(self.0.motion(motion()))
    }

    pub fn timing(self, timing: InsertionSortTiming) -> Self {
    ...
    pub fn build(self) -> (Box<dyn Playable>, Viewport) {
        ...
        let (state, algorithm, motion, timing) = self
            .0
            .take()
            .expect("insertion-sort: state, algorithm, motion required");
        build_insertion_sort(name, state, algorithm(state), motion, timing)
    }
