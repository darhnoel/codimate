use crate::{view::build_symspell, SymSpell, SymSpellMotion, SymSpellTiming, SymTrace};
...
impl ExplainBuilder {
    pub fn state(self, state: SymSpell) -> Self {
        Self(self.0.state(state))
    }

    pub fn algorithm(self, algorithm: fn(SymSpell) -> SymTrace) -> Self {
        Self(self.0.algorithm(algorithm))
    }

    pub fn motion(self, motion: fn() -> SymSpellMotion) -> Self {
        Self(self.0.motion(motion()))
    }

    pub fn timing(self, timing: SymSpellTiming) -> Self {
