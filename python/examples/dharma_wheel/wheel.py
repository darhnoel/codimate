...
def polar(centre, degrees, radius):
    ...
    return (centre[0] + radius * math.cos(radians),
...
def _finials(scene, centre, angle, spokes):
    ...
    for index in range(spokes):
        ...
        for lobe, (offset, reach, size) in enumerate(_LOBES):
            ...
            scene.circle(("finial", index, lobe, "edge"), r=size + _E,
                         at=spot).fill(EDGE).on(layer=1)
            scene.circle(("finial", index, lobe), r=size,
                         at=spot).fill(GOLD).on(layer=2)
    ...
    for index in range(spokes):
        ...
        for part, (a, b, wide) in enumerate(((hub_end, waist, SPOKE_W + 6.0),
            ...
            scene.line(("spoke", index, part, "edge"), start=a, end=b,
                       w=wide + (SPOKE_EDGE - SPOKE_W)).fill(EDGE).on(layer=7)
            scene.line(("spoke", index, part), start=a, end=b,
                       w=wide).fill(GOLD).on(layer=8)

        # The diamond block partway along — a short, very thick line.
        a, b = polar(centre, base, ORNAMENT_IN), polar(centre, base, ORNAMENT_OUT)
        scene.line(("ornament", index, "edge"), start=a, end=b,
                   w=24.0).fill(EDGE).on(layer=9)
        scene.line(("ornament", index), start=a, end=b,
                   w=17.5).fill(GOLD_DEEP).on(layer=10)
...
def _studs(scene, centre, angle, spokes):
    ...
    for index in range(spokes):
        ...
        scene.circle(("stud", index, "edge"), r=DOT_R + 2.5,
                     at=spot).fill(EDGE).on(layer=11)
        scene.circle(("stud", index), r=DOT_R, at=spot).fill(GOLD_DEEP).on(layer=12)
...
def _hub(scene, centre, angle):
    for name, radius, color, layer in (
        ...
        scene.circle(("hub", name), r=radius, at=centre).fill(color).on(layer=layer)

    # Three lobes turning together — a triskelion, as far as circles allow.
    for lobe in range(3):
        spot = polar(centre, angle * 1.5 + lobe * 120, 13.0)
        scene.circle(("swirl", lobe, "edge"), r=12.5, at=spot).fill(EDGE).on(layer=17)
        scene.circle(("swirl", lobe), r=9.5, at=spot).fill(GOLD).on(layer=18)
